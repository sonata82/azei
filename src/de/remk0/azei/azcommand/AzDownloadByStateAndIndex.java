...
public abstract class AzDownloadByStateAndIndex extends AzDownloadByState {
    ...
    public boolean execute(PluginInterface pluginInterface) {
        ...
        if (this.index == -1) {
            ...
            // change the status of download defined by state and index
            boolean searchForCompleted = (this.getState() == STATES.SEED);

            for (Download d : pluginInterface.getDownloadManager()
                    .getDownloads()) {
                if (d.getPosition() == this.index
                        && d.isComplete() == searchForCompleted) {
     ...
     * 
     * @see
     * de.remk0.azei.azcommand.IAzCommand#parseResult(org.gudy.azureus2.plugins
     * .PluginInterface)
     */
    @Override
    public String parseResult(boolean successfull,
        ...
        if (successfull) {
            ...
            if (this.index != -1) {
                return "command failed. " + Integer.toString(this.index)
                        + " = illegal index?\n"
